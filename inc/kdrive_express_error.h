...
#define KDRIVE_OBJSER_DESCRIPTOR_NOT_FOUND_ERROR		(0x000B)	/*!< The object server descriptor wasn't found */
#define KDRIVE_FORMATER_DESCRIPTOR_NOT_FOUND_ERROR		(0x000C)	/*!< The telegram formatter descriptor wasn't found */
...
#define KDRIVE_EASY_ERROR								(0x5000)	/*!* kdriveEasy error */
#define KDRIVE_EASY_OBJECT_SERVER_NOT_SET				(0x5001)	/*!* Object Server not set */
#define KDRIVE_EASY_ABORT_LINKING_AFTER_ERROR			(0x5002)	/*!* Abort linking procedure after error */
#define KDRIVE_EASY_ABORT_LINKING_AFTER_TIMEOUT			(0x5003)	/*!* Abort linking procedure after timeout */
...
extern "C" {
	...
	*/
	kdrive_API void kdrive_get_error_message(error_t e, char* str, uint32_t str_len);

	/*!
		The Error Callback definition
	*/
	typedef void (*kdrive_error_callback)(error_t e);

	/*!
		An error callback
	*/
	kdrive_API void kdrive_register_error_callback(kdrive_error_callback c);
